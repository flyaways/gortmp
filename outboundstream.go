...
func (stream *outboundStream) Play(streamName string, start, duration *uint32, reset *bool) (err error) {
	...
	stream.conn.Connection().SetStreamBufferSize(stream.id, stream.bufferLength)
	return nil
}
...
func (stream *outboundStream) onStatus(cmd *Command) bool {
	...
	case NETSTREAM_PLAY_START:
		// Set buffer size
		//stream.conn.Connection().SetStreamBufferSize(stream.id, 1500)
...
func (stream *outboundStream) Attach(handler StreamHandler) {
